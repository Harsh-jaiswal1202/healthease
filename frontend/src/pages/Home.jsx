...
const Home = () => {
  ...
  return (
    <div ref={containerRef} className="relative overflow-hidden min-h-screen bg-white dark:bg-gray-900 transition-colors duration-300">
      {/* Modern Healthcare UI Background Layers - Only render heavily animated backgrounds on desktop */}
      {!isMobile && (
          ...
          {[...Array(3)].map((_, i) => (
            <motion.div
              ...
              style={{
                width: `${300 + i * 200}px`,
                height: `${300 + i * 200}px`,
                background: `radial-gradient(circle, ${i === 0 ? 'rgba(147, 197, 253, 0.15)' :
      ...
      <div className="relative z-10">
        <Header />
        <div className={!isMobile ? "section-animate" : ""}>
          <SpecialityMenu />
        </div>
        <div className={!isMobile ? "section-animate" : ""}>
          <TopDoctors compact />
